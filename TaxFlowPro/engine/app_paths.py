...
def get_data_root() -> Path:
    ...
    restarts; running from source keeps using the TaxFlowPro app root (one level
    above this file, which lives in TaxFlowPro/engine/)."""
    if is_frozen():
...
def get_bundle_dir() -> Path:
    ...
    app folder itself for --onedir); running from source this is the TaxFlowPro
    app root (one level above this file, which lives in TaxFlowPro/engine/)."""
    if is_frozen():
